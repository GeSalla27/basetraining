class AppRoutes {
  static const HOME = '/home_page';
  static const USER_LIST = '/user_list';
  static const USER_FORM = '/user_form';
  static const USER_DETAIL = '/user_detail';
  static const TRAINING_REQUEST_LIST = '/training_request_list';
  static const TRAINING_REQUEST_FORM = '/training_request_form';
  static const TRAINING_REQUEST_DETAIL = '/training_request_detail';
  static const TRAINING_SCHEDULE_LIST = '/training_schedule_list';
  static const TRAINING_SCHEDULE_FORM = '/training_schedule_form';
  static const TRAINING_SCHEDULE_DETAIL = '/training_schedule_detail';
}
